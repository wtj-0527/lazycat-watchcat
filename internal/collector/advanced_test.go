...
func TestParseSmartATAOmitsNVMeMetrics(t *testing.T) {
	raw := []byte(`{"model_name":"WDC WD40EZAX-22C8UB0","serial_number":"WD-WX32D55JENEU","temperature":{"current":41},"power_on_time":{"hours":3200},"ata_smart_attributes":{"table":[{"name":"Reallocated_Sector_Ct","raw":{"value":1}},{"name":"Current_Pending_Sector","raw":{"value":2}},{"name":"Offline_Uncorrectable","raw":{"value":3}},{"name":"Reported_Uncorrect","raw":{"value":4}}]}}`)
	...
	for _, point := range points {
		...
		if strings.HasPrefix(point.Name, "disk.ata.") {
			seenATA[point.Name] = true
		}
	}
	for _, name := range []string{
		"disk.ata.reallocated_sectors", "disk.ata.pending_sectors",
		"disk.ata.offline_uncorrectable", "disk.ata.reported_uncorrectable",
	} {
		if !seenATA[name] {
			t.Fatalf("missing ATA metric %s", name)
		}
	}
}
