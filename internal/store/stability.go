...
func (s *Store) StabilityInputs(ctx context.Context) (StabilityInputs, error) {
	...
	if err := s.reader().QueryRowContext(ctx, `SELECT MAX(received_at) FROM metrics`).Scan(&latest); err != nil {
		return out, err
	}
	if latest.Valid {
	...
	if err := s.reader().QueryRowContext(ctx, `SELECT COUNT(*) FROM notification_outbox WHERE status='pending'`).Scan(&out.PendingNotifications); err != nil {
		return out, err
	}
	...
	var updated string
	if err := s.reader().QueryRowContext(ctx, `SELECT updated_at FROM system_state WHERE name='last_retention'`).Scan(&updated); err == nil {
