...
describe('OverviewPage', () => {
  ...
  it('does not let a safe root filesystem mask critical Btrfs usage', async () => {
    ...
    const realtimeButton = wrapper.get('.realtime-mode-button')
    expect(realtimeButton.text()).toContain('开启实时')
    await realtimeButton.trigger('click')
    expect(realtimeButton.text()).toContain('实时 · 5 秒')
    expect(realtimeButton.attributes('aria-pressed')).toBe('true')
