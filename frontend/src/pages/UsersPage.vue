...
<PageState :loading="loading" :error="error" :empty="data?.items.length===0" empty-title="尚无用户数据" empty-text="用户状态将在首次真实采集后显示。" @retry="refresh">
  ...
  <div class="user-layout">
    <section class="card user-list"><button v-for="item in filtered" :key="`${item.deviceId}-${item.userId}`" :class="{active:selected?.deviceId===item.deviceId&&selected?.userId===item.userId}" @click="selectedKey=`${item.deviceId}\0${item.userId}`"><span class="user-list-avatar">{{(item.nickname||item.userId).slice(0,1).toUpperCase()}}</span><span><b>{{item.nickname||item.userId}}</b><small>{{item.deviceName}} · {{item.userId}}</small></span><StatusPill :status="item.online?'healthy':'unknown'" /></button></section>
    <section v-if="selected" class="card user-detail">
      <div class="user-detail-head"><div class="user-list-avatar large">{{(selected.nickname||selected.userId).slice(0,1).toUpperCase()}}</div><div><h2>{{selected.nickname}}</h2><p>{{selected.userId}} · {{selected.deviceName}}</p></div><span class="pill" :class="selected.online?'healthy':'unknown'">{{selected.online?'在线':'离线'}}</span></div>
