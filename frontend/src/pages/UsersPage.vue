...
watch(userPagination.page,()=>{const items=userPagination.pagedItems.value;if(items.length&&!items.some(item=>`${item.deviceId}\0${item.userId}`===selectedKey.value))selectedKey.value=`${items[0].deviceId}\0${items[0].userId}`})
...
function routeState(){
  ...
  const tab=params.get('tab') as UserTab|null
  return {deviceId:params.get('device')||'',userId:params.get('user')||'',tab:userTabs.some(item=>item.id===tab)?tab||'overview':'overview'}
}
function syncUserRoute(){
...
function updateUserRoute(){
  ...
  history.replaceState(null,'',`${location.pathname}${location.search}#users?${params}`)
}
function selectUser(item:UserItem){selectedKey.value=`${item.deviceId}\0${item.userId}`;mobileDetail.value=true}
function selectTab(tab:UserTab){activeTab.value=tab}
function showUserList(){mobileDetail.value=false}
...
<PageState :loading="loading" :error="error" :empty="data?.items.length===0" empty-title="尚无用户数据" empty-text="用户状态将在首次真实采集后显示。" @retry="refresh">
  ...
  <div class="user-layout" :class="{'mobile-detail':mobileDetail}">
    ...
    <section v-if="selected" class="card user-detail">
      <button class="mobile-user-back" @click="showUserList"><AppIcon name="arrow-left" :size="16"/>返回用户列表</button>
      <div class="user-detail-sticky">
        ...
        <div class="user-detail-tabs" role="tablist" aria-label="用户详情分类"><button v-for="tab in userTabs" :key="tab.id" role="tab" :aria-selected="activeTab===tab.id" :class="{active:activeTab===tab.id}" @click="selectTab(tab.id)">{{tab.label}}<span v-if="tab.id==='endpoints'">{{selected.totalDevices}}</span><span v-else-if="tab.id==='history'">{{selected.loginCount}}</span></button></div>
      </div>

      <section v-if="activeTab==='overview'" class="user-tab-panel">
      ...
      <section v-else-if="activeTab==='access'" class="user-tab-panel">
      ...
      <section v-else-if="activeTab==='endpoints'" class="user-tab-panel">
      ...
      <section v-else class="user-tab-panel">
